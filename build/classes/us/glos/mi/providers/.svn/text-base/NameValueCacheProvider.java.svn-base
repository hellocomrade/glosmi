...
public class NameValueCacheProvider extends TemporalCacheBase<List<NameValuePair>> {
	INameValueDAO<NameValuePair> dao=null;
	private String CACHE_DURATION_NAME="CACHE_DURATION";
	private String DAO_NAME="DAO_NAME";
	private List<NameValuePair> readonlyList;
	public NameValueCacheProvider(String key)
	{
		init(key);
	}
	public void init(String key)
	{
		...
		try
		{
			temp=ConfigManager.getProviderParam(key,CACHE_DURATION_NAME);
			if(temp==null||temp.equals(""))
			{
				Logger.writeLog("NameValueCacheProvider: CACHE_DURATION can not be null", LogLevel.SEVERE);
				throw new NullPointerException("NameValueCacheProvider: CACHE_DURATION can not be null");
			}
			else
			{
				try
				...
				{
					Logger.writeLog("NameValueCacheProvider: Can not cast CACHE_DURATION to Long.",LogLevel.SEVERE);
					throw nfe;
				}
			}
			temp=ConfigManager.getProviderParam(key,DAO_NAME);
			if(temp!=null)
			{
			...
			{
				Logger.writeLog("NameValueCacheProvider: DAO_NAME can not be null", LogLevel.SEVERE);
				throw new NullPointerException("NameValueCacheProvider: DAO_NAME can not be null");
			}
		}
	...
	protected synchronized void run() {
		...
		if(dao!=null)
		{
			try
			{
				...
				Logger.writeLog("NameValueCacheProvider:run: Failed to update cached data",LogLevel.SEVERE);
