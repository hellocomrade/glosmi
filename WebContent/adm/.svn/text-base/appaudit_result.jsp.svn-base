...
String val=null;
String msg=null;
if(session.getAttribute("APP_AUDIT_RESULT_VALUE")!=null&&!session.getAttribute("APP_AUDIT_RESULT_VALUE").equals("")
   &&session.getAttribute("APP_AUDIT_RESULT_MESSAGE")!=null&&!session.getAttribute("APP_AUDIT_RESULT_MESSAGE").equals(""))
{
	val=session.getAttribute("APP_AUDIT_RESULT_VALUE").toString();
	msg=session.getAttribute("APP_AUDIT_RESULT_MESSAGE").toString();
	
	session.removeAttribute("APP_AUDIT_RESULT_VALUE");
	session.removeAttribute("APP_AUDIT_RESULT_MESSAGE");
}
else
{
...
<%
if(literals!=null)
	out.println(String.format("%s%s",literals.getText("title"),literals.getText("/adm/appaudit_result.jsp")));
else
	out.println("Application Audit Result");
%>
</title>
