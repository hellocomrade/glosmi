...
String val=null;
String msg=null;
if(session.getAttribute("MODEL_SUBMITTAL_RESULT_VALUE")!=null&&!session.getAttribute("MODEL_SUBMITTAL_RESULT_VALUE").equals("")
   &&session.getAttribute("MODEL_SUBMITTAL_RESULT_MESSAGE")!=null&&!session.getAttribute("MODEL_SUBMITTAL_RESULT_MESSAGE").equals(""))
{
	val=session.getAttribute("MODEL_SUBMITTAL_RESULT_VALUE").toString();
	msg=session.getAttribute("MODEL_SUBMITTAL_RESULT_MESSAGE").toString();
	
	session.removeAttribute("MODEL_SUBMITTAL_RESULT_VALUE");
	session.removeAttribute("MODEL_SUBMITTAL_RESULT_MESSAGE");
}
else
{
...
<%
if(literals!=null)
	out.println(String.format("%s%s",literals.getText("title"),literals.getText("/mod/result.jsp")));
else
	out.println("Model Submittal Result");
%>
</title>
...
{
%>
	<div>Click here to start adding new model</div>
<%
}
%>
</div>
