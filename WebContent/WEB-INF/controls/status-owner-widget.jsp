...
if(usr==null&&!(UserPrivilegeMask.IsAdmin(usr.getPrivilegeLevel())))
	return;
%>
<jsp:useBean id="sow_modparam" scope="page" class="us.glos.mi.domain.ModAdminParam" />
...
$("#chown_btn").click(function(){
	if($.trim($("input#chown_email").val()).length>0&&true==validateEmail($("input#chown_email").val()))
	{
		...
		$.get(chown_url,
				{
					<%=sow_modparam.getModEmailParamName()%>:$.trim($("input#chown_email").val())
				},
				function(data){
					if(data=="0")
					{
...
{
	chstas_url=url;
	$.get("../adm/modstatus.glos",
			{
				<%=sow_modparam.getModIdParamName()%>:id
			},
			function(data){
				if(data=="0")
				{
...
$("#chstas_btn").click(function(){
	...
	$.get(chstas_url,
			{
				<%=sow_modparam.getModStatusParamName()%>:$("[name=chstas_radio]:checked").val()
			},
			function(data){
				if(data=="0")
				{
