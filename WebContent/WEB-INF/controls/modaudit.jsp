...
<%
if(literals!=null)
	out.println(String.format("%s%s",literals.getText("title"),literals.getText("modaudit.jsp")));
else
	out.println("Model Audit");
%>
</title>
	...
	$(function() {
		...
		$("#a_modother_next").button();
		$("#a_modaudit_prev").button();
		...
		$("#a_modother_next").click(function(event){
			  //event.preventDefault();
			  $("#mod_tabs").tabs('select',3);
		});
		$("#a_modaudit_prev").click(function(event){
			  //event.preventDefault();
			  $("#mod_tabs").tabs('select',2);
		});
		$("#a_mod_submit").click(function(event){
			event.preventDefault();
			if(true==validate_form())
			{
				$("[name=mod_frm]").attr("action","modaudit.glos");

				$("#mod_action").val("approve");
...
<div style="margin-left:auto;margin-right:auto;width:810px;">
	...
	<div id="mod_tabs">
		...
		<input type="hidden" id="mod_id" name="<%=modparam.getModIdParamName()%>" value="<%=model.getId()%>" />
		<input type="hidden" id="draft_id" name="<%=modparam.getDraftIdParamName() %>" value="<%=model.getSerialId()%>" />
...
%>
					...
					<textarea class="textbox-app" rows="10" cols="90"><%=(model==null)?empty:HTMLUtilities.filterDisplay(model.getUpdateReason())%></textarea>
<%
}
%>
				<fieldset>
					<legend class="fd_legend">Model Category (Check all that apply):</legend>
					<glos:checkboxdisplay keyValues="<%=DBCache.getModelCategories() %>" namePrefix="<%=modparam.getModCategoryParamName() %>" divId="mod_cat_div" checkedIds="<%=Converter.ToStringList(model.getCategoryIds())%>"/>
<%
String errMsg=null;
if(err!=null)
...
%>
						<input type="text" name="<%=modparam.getModNameParamName() %>" id="mod_name" style="" class="<%=errMsg==null?"textbox-app":"textbox-error"%>" size="50" value="<%=HTMLUtilities.filterDisplay(model.getName())%>"/>
						<div id="err_mod_name" style="color:red;display:inline;"><%=errMsg==null?empty:errMsg %></div>
					</div>
							
					<div style="">Acronym:</div>
					<div>
						<input type="text" name="<%=modparam.getModAbbrevParamName() %>" id="mod_abbrev" class="textbox-app" size="50" value="<%=HTMLUtilities.filterDisplay(model.getAbbreviation())%>"/>
			        </div>
<%
if(err!=null)
...
%>			   
				...
				<br/>
				<fieldset>
					<legend class="fd_legend">Current Version (Current version of model):</legend>
<%
if(err!=null)
...
%>
					<glos:radiodisplay keyValues="<%=DBCache.getModelAvailibilites() %>" name="<%=modparam.getModAvailParamName() %>" idPrefix="mod_avail_radio" divId="mod_avail_div" checkedId="<%=model.getAvailableId()%>"/>
...
%>
					<glos:checkboxdisplay keyValues="<%=DBCache.getThemes() %>" namePrefix="<%=modparam.getModThemeParamName() %>" divId="mod_theme_div" checkedIds="<%=Converter.ToStringList(model.getThemeIds())%>"/>
				...
				<fieldset>
					...
					<div style="font-size:smaller">
						...
						whether use is management or research or both.)
					</div>
<%
if(err!=null)
	errMsg=err.getErrorMsg(modparam.getModDescParamName());
%>
						<textarea name="<%=modparam.getModDescParamName() %>" id="mod_purpose" class="<%=errMsg==null?"textbox-app":"textbox-error"%>" rows="10" cols="90"><%=HTMLUtilities.filterDisplay(model.getDescription())%></textarea>
...
%>
					<textarea name="<%=modparam.getModAttriParamName() %>" id="mod_attri" class="<%=errMsg==null?"textbox-app":"textbox-error"%>" rows="10" cols="90"><%=HTMLUtilities.filterDisplay(model.getAttribute())%></textarea>
...
%>
					<textarea name="<%=modparam.getModDataReqParamName() %>" id="mod_datareq" class="<%=errMsg==null?"textbox-app":"textbox-error"%>" rows="10" cols="90"><%=HTMLUtilities.filterDisplay(model.getDataRequirement())%></textarea>
							...
							<tr>
								<td colspan="3">
									...
									<fieldset>
										...
										<div>
											<select name="<%=cntparam.getDeveloperParamName()+cntparam.getContactFlag() %>" class="loginfont textbox-app">
												<option value="<%=cntparam.getNewContactVal() %>" <%=isnewmod?"selected":empty %>>New Contact(Insert)</option>
												<option value="<%=cntparam.getUpdateContactVal() %>">Update Existing Contact(Update)</option>
												<option value="<%=cntparam.getExistContactVal() %>" <%=!isnewmod?"selected":empty %>>Existing Contact(No Action)</option>
											</select>
										</div>
									</fieldset>
								</td>
							...
							<tr>
								<td colspan="3">
									<fieldset>
										<legend>User Update Type</legend>
										<div>
											<select name="<%=cntparam.getDistributorParamName()+cntparam.getContactFlag() %>" class="loginfont textbox-app">
												<option value="<%=cntparam.getNewContactVal() %>" <%=isnewmod?"selected":empty %>>New Contact(Insert)</option>
												<option value="<%=cntparam.getUpdateContactVal() %>">Update Existing Contact(Update)</option>
												<option value="<%=cntparam.getExistContactVal() %>" <%=!isnewmod?"selected":empty %>>Existing Contact(No Action)</option>
											</select>
						...
						<tr>
							<td colspan="2">
								<fieldset>
									<legend>User Update Type</legend>
									<div>
										<select name="<%=modparam.getModContactFlagParamName() %>" class="loginfont textbox-app">
											<option value="<%=cntparam.getNewContactVal() %>" <%=isnewmod?"selected":empty %>>New Contact(Insert)</option>
											<option value="<%=cntparam.getUpdateContactVal() %>">Update Existing Contact(Update)</option>
											<option value="<%=cntparam.getExistContactVal() %>" <%=!isnewmod?"selected":empty %>>Existing Contact(No Action)</option>
										</select>
...
%>
				<textarea name="<%=modparam.getModEasyUseParamName() %>" id="mod_easyuse" class="textbox-app" rows="10" cols="90"><%=HTMLUtilities.filterDisplay(model.getSkillLevel()) %></textarea>
				<br/><div id="err_mod_easyuse" style="color:red;display:inline;"><%=errMsg==null?empty:errMsg %></div>
			</fieldset>
			<br/>
			<fieldset>
				<legend class="fd_legend">Strength and Weaknesses:</legend>
				<div style="font-size:smaller">
				...
				<textarea name="<%=modparam.getModStrengthParamName() %>" id="mod_stren" class="textbox-app" rows="10" cols="90"><%=HTMLUtilities.filterDisplay(model.getStrength())%></textarea>
			</fieldset>
			<br/>
			<fieldset>
				...
				<textarea name="<%=modparam.getModNoteParamName() %>" id="mod_note" class="textbox-app" rows="10" cols="90"><%=HTMLUtilities.filterDisplay(model.getNote())%></textarea>
			</fieldset>
			<br/>
				
			<a id="a_modother_prev" href="#" style="font-size:larger;">Prev Page</a>
			<a id="a_modother_next" href="#" style="font-size:larger;">Next Page</a>
		</div>
		<div id="modaudit">
			<p>
				This model is submitted by <% out.print(HTMLUtilities.filterDisplay(model.getOwnerName())); %> (<% out.print(HTMLUtilities.filterDisplay(model.getOwnerEmail()));%>)&nbsp;
<%
	if(model.getSerialId()>0)
	{
		if(model.getId()>0)
			out.println(String.format("based upon an EXISTING model:<a href='./modaudit.glos?%s=update&%s=%d' target='_blank'><span style='font-weight:bold;'>%s</span></a>.",
		                          modparam.getActionParamName(),modparam.getModIdParamName(),model.getId(),model.getName()));
		else
			out.println("as a NEW model.");
	}
	else
	{
...
%>
			...
			<fieldset>
				...
				<input type="radio" name="<%=modparam.getModApprovalFlagParam() %>" value="<%=modparam.getNewModelFlag() %>" <%=isnewmod?"checked":empty %>/><span>Approve as a new model</span><br/>
				<input type="radio" name="<%=modparam.getModApprovalFlagParam() %>" value="<%=modparam.getUpdateModelFlag() %>" <%if(model.getId()<=0)out.print("disabled"); %> <%=!isnewmod?"checked":empty %>></input><span>Approve as an update on the existing model</span><br/>
			</fieldset>
			<br/>
			<a id="a_modaudit_prev" href="#" style="font-size:larger;">Prev Page</a>
			<a id="a_mod_submit_btn" href="#" style="font-size:larger;">Approve</a>
		</div>
		</form>
	</div>
</div>
<div style="height:300px;width:80px;top:2px;right:3px;position:absolute;" class="ui-widget-header ui-corner-all">
	<table align="center">
		<tr>
			<td>
				<a href="#" id="a_mod_submit" title="Approve"></a>
			</td>
		</tr>
		<tr>
			<td>
							...
							<tr>
								<td colspan="3">
									<fieldset>
										...
										<div>
											<select name="<%=cntparam.getDeveloperParamName()+cntparam.getContactFlag() %>" class="loginfont textbox-app">
												<option selected="selected" value="<%=cntparam.getNewContactVal() %>">New Contact(Insert)</option>
												<option value="<%=cntparam.getUpdateContactVal() %>">Update Existing Contact(Update)</option>
												<option value="<%=cntparam.getExistContactVal() %>">Existing Contact(No Action)</option>
											</select>
										</div>
									</fieldset>
								</td>
							...
							<tr>
								<td colspan="3">
									<fieldset>
										...
										<div>
											<select name="<%=cntparam.getDistributorParamName()+cntparam.getContactFlag() %>" class="loginfont textbox-app">
												<option value="<%=cntparam.getNewContactVal() %>" selected="selected">New Contact(Insert)</option>
												<option value="<%=cntparam.getUpdateContactVal() %>">Update Existing Contact(Update)</option>
												<option value="<%=cntparam.getExistContactVal() %>">Existing Contact(No Action)</option>
											</select>
										</div>
